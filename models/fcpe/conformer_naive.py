...
# Ported verbatim from FCPE (https://github.com/CNChTu/FCPE) torchfcpe/model_conformer_naive.py
# Original derived from https://github.com/CNChTu/Diffusion-SVC/ by CNChTu (License: MIT)
#
# Only change vs upstream: the `einops` and `local_attention` imports are guarded so that the
# default `conv_only=True` path (no attention) imports and runs without those optional packages.
try:
    from einops import rearrange, repeat
except ImportError:  # only needed when attention is enabled (conv_only=False)
    rearrange = repeat = None

try:
    from local_attention import LocalAttention
except ImportError:  # only needed when local attention heads are used (local_heads > 0)
    LocalAttention = None
