...
class PulseGen(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, f0: torch.Tensor, mel: torch.Tensor, hop_length: int) -> torch.Tensor:
        """Generate one-sample pulses guided by f0 with intensity from mel.

        Args:
            f0 (torch.Tensor): [B, 1, T_audio]
            mel (torch.Tensor): [B, n_mels, T_mel]
            hop_length (int): hop to expand mel to samples
        Returns:
            torch.Tensor: [B, 1, T_audio]
        """
        B, _, T = f0.shape

        # Upsample mel to sample domain and compute intensity as mean across bins
        mel_up = F.interpolate(mel, size=T, mode="nearest")
        intensity = mel_up.mean(dim=1, keepdim=True)

        # Normalize intensity roughly to [-1, 1] scale
        inten = intensity

        # Saw phase and pulse detection
        phase = torch.cumsum(f0 / self.sampling_rate, dim=2) % 1.0
        dphase = F.pad(phase[:, :, 1:] - phase[:, :, :-1], (1, 0))
        pulses = (dphase < 0).float()

        uv = (f0 > self.voiced_threshold).float()
        pulses = pulses * uv

        # Amplitude-scale pulses by intensity
        tpl = pulses * inten

        # Add light noise in unvoiced regions
        noise = torch.randn_like(tpl) * self.noise_std
        tpl = tpl + (1 - uv) * noise
        return tpl
...
class RefineGANGenerator(nn.Module):
    def __init__(
        ...
        *,
        ...
        elif template_generator == "pulse":
            self.template_gen = PulseGen(sampling_rate=sampling_rate)
        else:
